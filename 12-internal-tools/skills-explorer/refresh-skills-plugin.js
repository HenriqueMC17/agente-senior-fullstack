...
async function syncWithArchive() {
    ...
    try {
        ...
        const destSkills = path.join(ROOT_DIR, '09-skills');
        const destIndex = path.join(ROOT_DIR, '05-knowledge-base', 'registry', 'skills_index.json');
        ...
        // Note: For now, merging is safer than replacing in case agent has local skills
    ...
    return {
        ...
        configureServer(server) {
            ...
            server.middlewares.use('/skills.json', (req, res, next) => {
                const filePath = path.join(ROOT_DIR, '05-knowledge-base', 'registry', 'skills_index.json');
            ...
            // Serve /skills/* directly from ROOT_DIR/09-skills/
            server.middlewares.use((req, res, next) => {
                if (!req.url || !req.url.startsWith('/skills/')) return next();

                const relativePath = decodeURIComponent(req.url.replace(/^\/skills\//, '09-skills/').replace(/\?.*$/, ''));
                const filePath = path.join(ROOT_DIR, relativePath);

                const resolved = path.resolve(filePath);
                if (!resolved.startsWith(path.join(ROOT_DIR, '09-skills'))) return next();
            ...
            server.middlewares.use('/api/refresh-skills', async (req, res) => {
                ...
                try {
                    ...
                    // Count skills
                    const indexPath = path.join(ROOT_DIR, '05-knowledge-base', 'registry', 'skills_index.json');
                    let count = 0;
